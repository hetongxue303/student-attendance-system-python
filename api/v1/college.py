...
@router.get('/getAll', response_model=Success[Page[List[CollegeDto]]], summary='查询学院(All)',
            dependencies=[Security(check_permissions, scopes=['college:list'])])
...
@router.delete('/delete/{id}', response_model=Success[typing.Any], summary='删除学院',
               dependencies=[Security(check_permissions, scopes=['college:delete'])])
...
@router.put('/update', response_model=Success[typing.Any], summary='修改学院',
            dependencies=[Security(check_permissions, scopes=['college:update'])])
